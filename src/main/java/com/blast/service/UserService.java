...
import com.blast.config.Constants;
import com.blast.repository.UserRepository;
import com.blast.security.AuthoritiesConstants;
import com.blast.security.SecurityUtils;
import com.blast.service.util.RandomUtil;
...
public class UserService {
    ...
    public User getCurrentUser() {
    	...
    	if (optUser == null || optUser.get() == null) {
    		return null;
    	}
    ...
    @Transactional(readOnly = true)
    public UserDTO getUserDTOWithDisplayName(Long id) {
        ...
        List<SocialUserConnection> lstConnection = socialUserConnectionRepository.findAllByUserIdOrderByProviderIdAscRankAsc(user.getLogin());
    ...
    @Transactional(readOnly = true)
    public List<Long> getAllFollowingUserId(Long userId) {
    	// Get list following
		UserDTO userDTO = this.getUserDTOWithDisplayName(userId);
		
		if (StringUtils.isEmpty(userDTO.getProviderId())) {
			return null;
		}
		Set<String> lstProviderUserIdFollowing = new HashSet<>();
		List<Long> lstUserId = new ArrayList<>();
		
		List<SocialFollowDTO> lstFollowing = socialFollowService.getFollowing(userDTO.getProviderId(), userDTO.getProviderUserId());
		if (lstFollowing != null && lstFollowing.size() > 0) {
			for (SocialFollowDTO socialFollowDTO : lstFollowing) {
				lstProviderUserIdFollowing.add(socialFollowDTO.getFollowUserId());
			}
			
			// get list connection
			List<SocialUserConnection> lstSocialUser = socialUserConnectionRepository.findAllByProviderIdAndProviderUserIdIn(userDTO.getProviderId(), lstProviderUserIdFollowing);
			if (lstSocialUser != null) {
				for (SocialUserConnection socialUserConnection : lstSocialUser) {
					 Optional<com.blast.domain.User> userOpt = this.getUserWithAuthoritiesByLogin(socialUserConnection.getUserId());
					 if (userOpt != null && userOpt.get() != null) {
						 lstUserId.add(userOpt.get().getId());
					 }
				}
			}
		}
