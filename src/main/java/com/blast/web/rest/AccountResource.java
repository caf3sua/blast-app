...
import com.blast.service.MailService;
import com.blast.service.UserService;
import com.blast.service.dto.UserDTO;
import com.blast.web.rest.vm.KeyAndPasswordVM;
import com.blast.web.rest.vm.ManagedUserVM;
...
public class AccountResource {
     ...
     */
    @ApiOperation(value = "BLAST - Lấy thông tin của người dùng hiện tại")
    @GetMapping("/account")
    @Timed
    public ResponseEntity<UserDTO> getAccount() {
    	final String userLogin = SecurityUtils.getCurrentUserLogin();
        Optional<User> existingUser = userRepository.findOneWithAuthoritiesByLogin(userLogin);
        if (existingUser == null || existingUser.get() == null) {
    ...
    /**
     * POST  /account/change_display_name : changes the current user's password
     *
     * @param displayName
     * @return the ResponseEntity with status 200 (OK), or status 400 (Bad Request) if the new password is not strong enough
     */
    @PostMapping(path = "/account/change_display_name",
        produces = MediaType.TEXT_PLAIN_VALUE)
    @Timed
    public ResponseEntity changeDisplayName(@RequestBody String displayName) {
        if (StringUtils.isEmpty(displayName)) {
            return new ResponseEntity<>("Incorrect displayName", HttpStatus.BAD_REQUEST);
        }
        ...
        if (lstConnection != null && lstConnection.size() != 0) {
        	...
        	socialUserConnectionRepository.save(connection);
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
